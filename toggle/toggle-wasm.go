...
.vecty-form-check-toggle {
	position: relative;
	padding-left: 0;
	line-height: 30px;
}

.vecty-form-check-toggle input {
	display: block;
	position: absolute;
	top: 0;
	right: 0;
...
.vecty-form-check-toggle input + span {
	cursor: pointer;
	user-select: none;
	height: 30px;
	margin-left: 70px;
...
.vecty-form-check-toggle input + span:before {
	content: "";
	position: absolute;
	left: 0;
...
.vecty-form-check-toggle input + span:after {
	width: 28px;
	height: 28px;
	margin-top: 1px;
	margin-left: 1px;
...
.vecty-form-check-toggle input:checked + span:after {
	content: "";
	margin-left: 21px;
	box-shadow: none;
}

.vecty-form-check-toggle input:checked + span:before {
...
func (c *Toggle) Render() vecty.ComponentOrHTML {
	return elem.Div(
		vecty.Markup(
			vecty.Class("vecty-form-check"),
		),
		elem.Label(
			vecty.Markup(
				vecty.ClassMap{
					"vecty-form-check-label":  true,
					"vecty-form-check-toggle": true,
				},
			),
			elem.Input(
				vecty.Markup(
					vecty.Class("vecty-form-check-input"),
					prop.Type("checkbox"),
